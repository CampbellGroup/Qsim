...
class TimeHarpClient(QtGui.QWidget):
    ...
    def connect(self):
        ...
        yield self.server.addListener(
            listener=self.update_sync_count, source=None, ID=SIGNALID1
        )
        yield self.server.addListener(
            listener=self.update_count_rate, source=None, ID=SIGNALID2
        )
        yield self.server.addListener(
            listener=self.update_warnings, source=None, ID=SIGNALID3
        )
        self.initialize_GUI()
    ...
    def initialize_GUI(self):
        ...
        self.settings = [
            "picoharp_binning",
            "picoharp_channel_level",
            "picoharp_channel_zerox",
            "picoharp_measure_time",
            "picoharp_sync_level",
            "picoharp_sync_zerox",
            "picoharp_sync_divider",
            "picoharp_count_offset",
            "picoharp_sync_offset",
            "picoharp_mode",
            "picoharp_histo_iterations",
        ]
        ...
        print
        "creating spin boxes..."
        self.sync_level_spinbox = timeharp_spinbox(
            -800, 0, self.change_sync, init_settings[4], "mV"
        )
        self.sync_zero_spinbox = timeharp_spinbox(
            -40, 0, self.change_sync, init_settings[5], "mV"
        )
        self.count_level_spinbox = timeharp_spinbox(
            -800, 0.0, self.change_count, init_settings[1], "mV"
        )
        self.count_zero_spinbox = timeharp_spinbox(
            -40, 0.0, self.change_count, init_settings[2], "mV"
        )
        self.measure_time_spinbox = timeharp_spinbox(
            1.0, 10000.0, self.change_measure_time, init_settings[3], "ms"
        )
        self.count_offset_spinbox = timeharp_spinbox(
            0.0, 10000.0, self.change_count_offset, init_settings[7], "ns"
        )
        self.sync_offset_spinbox = timeharp_spinbox(
            0.0, 100000.0, self.change_sync_offset, init_settings[8], "ps"
        )
        self.histo_iter_spinbox = timeharp_spinbox(
            0.0, 50000.0, self.change_histo_iterations, init_settings[10]
        )
        ...
        self.histo_length_dropdown.addItems(
            ["25.6 ns", "51.2 ns", "102.4 ns", "204.8 ns", "409.6 ns", "819.2 ns"]
        )
    ...
    def on_hist_pressed(self, value):
        ...
        time.sleep(measure_time / 1000.0)
        yield self.server.stop_measure()
        data = yield self.server.get_histogram(0, 1, data_length)
        hist = np.column_stack((binning * 25 * np.array(bins) / 1000.0, data))
        hist = hist.astype(float)
        total_hist.append(sum(hist[:, 1]))

        #       for hist in hist_array:
        self.dataset_hist = yield self.data_vault.new(
            "Histogram", [("run", "arb u")], [("Counts", "Counts", "num")]
        )
        bins = []
    ...
    def on_timetags_pressed(self, value):
        ...
        timetags = []
        for datum in data:
            timetag = ((dtime_mask & datum) >> 10) * 25.0 / 1000.0
