...
from Qsim.scripts.pulse_sequences.metastable_three_preparation import (
    MetastableThreePreparation as metastable_bright_sequence,
)  # this is where I define which state is bright
    ...
    def run(self, cxn, context):
        ...
        while True:
            ...
            all_bright_errors = np.unique(
                np.concatenate(
                    (
                        failed_three_heralding_bright[0],
                        failed_four_heralding_bright[0],
                        doppler_errors_bright[0],
                    )
                )
            )
            ...
            [counts_doppler_dark, counts_herald_dark, counts_dark] = self.run_sequence(
                max_runs=333, num=3
            )
