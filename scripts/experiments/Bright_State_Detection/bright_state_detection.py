import labrad
from Qsim.scripts.pulse_sequences.bright_state_preparation import (
    BrightStatePreparation as sequence,
)
...
class BrightStateDetection(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        while True:
            ...
            if mode == "Shelving":
                ...
                bright_dataset = np.column_stack(
                ...
                print(
                    "Mean detection counts on experiment "
                    + str(i)
                    + " = "
                    + str(np.mean(counts))
                )
            ...
            # run and process data if detection mode is standard
                ...
                bright_dataset = np.column_stack(
                    (np.arange(len(counts)), np.arange(len(counts)), np.array(counts))
                )
                self.dv.add(bright_dataset, context=self.bright_state_counts)
            ...
            if mode == "Shelving":
                if i == 1:
                    ...
                    print(
                        "mean detection counts on first experiment is  = "
                        + str(self.hist_mean)
                    )
                elif i > 1:
                    ...
                    if np.abs(diff) > 6.0:
                        self.cavity_voltage = (
                            self.cavity_voltage + np.sign(diff) * 0.005
                        )
                        self.pzt_server.set_voltage(
                            self.cavity_chan, U(self.cavity_voltage, "V")
                        )
                        print(
                            "Updated cavity voltage to "
                            + str(self.cavity_voltage)
                            + " V"
                        )
                    else:
                        pass
    ...
    def setup_prob_datavault(self):
        ...
        self.bright_state_counts_dataset = self.dv.new(
            "bright_state_counts",
            [("run", "arb")],
            [
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.bright_state_counts
            )
