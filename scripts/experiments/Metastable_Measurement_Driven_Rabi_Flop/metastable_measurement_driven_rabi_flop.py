...
class MetastableMeasurementDrivenRabiFlop(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        self.num_pulses = self.get_scan_list(
            self.p.MetastableMeasurementDrivenGate.scan, ""
        )
        for i, pulse_num in enumerate(self.num_pulses):
            ...
            all_errors = np.unique(
                np.concatenate((failed_heralding[0], doppler_errors[0], failed_gate[0]))
            )
