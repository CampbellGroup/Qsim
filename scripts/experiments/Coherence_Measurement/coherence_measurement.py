import labrad
from Qsim.scripts.pulse_sequences.coherence_measurement_point import (
    CoherenceMeasurementPoint as sequence,
)
...
class CoherenceMeasurement(QsimExperiment):
                ...
                self.dv.add(
                    np.column_stack(
                        (
                            ...
                            np.array(detection_counts),
                            np.array(doppler_counts),
                        )
                    ),
    ...
    def setup_coherence_shelving_datavault(self):
        ...
        self.dv.cd(
            ["coherence_measurement", "shelving_counts"],
            True,
            context=self.counts_context,
        )

        self.coherence_counts_dataset = self.dv.new(
            "counts",
            [("run", "arb")],
            [
