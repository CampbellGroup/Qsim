    ...
    def run(self, cxn, context):
        ...
        if self.p["LaserMonitor.laser"] in ["369", "399"]:
            ...
            if self.p["LaserMonitor.laser"] == "shelving lasers":
                ...
                self.initfreq = self.wlm.get_frequency(
                    self.chan_dict[str(self.p["LaserMonitor.laser"])]
                )
                print(self.initfreq)
        ...
        while (time.time() - self.inittime) <= self.p["LaserMonitor.measure_time"]["s"]:
            ...
            if self.p["LaserMonitor.laser"] in ["369", "399"]:
                while (time.time() - init_averaging_time) < self.p[
                    "LaserMonitor.averaging_time"
                ]["s"]:
                ...
                if self.p["LaserMonitor.laser"] == "shelving lasers":
                    ...
                    while (time.time() - init_averaging_time) < self.p[
                        ...
                        freq_list_760_1.append(
                            self.wlm.get_frequency(self.chan_dict["760_1"])
                        )
                        freq_list_760_2.append(
                            self.wlm.get_frequency(self.chan_dict["760_2"])
                        )
                        freq_list_822.append(
                            self.wlm.get_frequency(self.chan_dict["822"])
                        )
            ...
            if self.p["LaserMonitor.laser"] == "shelving lasers":
                freq_760_1, freq_760_2, freq_822 = (
                    np.mean(freq_list_760_1),
                    np.mean(freq_list_760_2),
                    np.mean(freq_list_822),
                )
            else:
            ...
            try:
                if self.p["LaserMonitor.laser"] == "shelving lasers":
                    self.dv.add(
                        ...
                        1e6 * (freq_760_1 - self.initfreq_760_1),
                        1e6 * (freq_760_2 - self.initfreq_760_2),
                        1e6 * (freq_822 - self.initfreq_822),
                        context=self.shelving_laser_context,
                    )
                else:
                    self.dv.add(
                        time.time() - self.inittime, 1e6 * (freq - self.initfreq)
                    )
            except:
                pass

            progress = (
                float(time.time() - self.inittime)
                / self.p["LaserMonitor.measure_time"]["s"]
            )
        ...
        if self.p["LaserMonitor.laser"] == "shelving lasers":
            ...
            for parameter in self.p:
                self.dv.add_parameter(
                    parameter, self.p[parameter], context=self.shelving_laser_context
                )
