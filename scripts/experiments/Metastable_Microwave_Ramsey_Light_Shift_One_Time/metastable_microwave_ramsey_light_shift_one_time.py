...
class OneTimeMetastableMicrowaveRamseyLightShift(QsimExperiment):
    """ """

    name = "One Time Metastable Microwave Ramsey Light Shift"

    exp_parameters = []
    exp_parameters.append(("MetastableMicrowaveRamseyLightShift", "interleaved_period"))
    exp_parameters.append(("OneTimeMetastableMicrowaveRamseyLightShift", "delay_time"))
    exp_parameters.append(("OneTimeMetastableMicrowaveRamseyLightShift", "reps"))
    ...
    def run(self, cxn, context):
        ...
        if scan_parameter == "delay_time":
            ...
            dark_time = self.p["OneTimeMetastableMicrowaveRamseyLightShift.delay_time"][
                "ms"
            ]

            self.p["EmptySequence.duration"] = self.p[
                "OneTimeMetastableMicrowaveRamseyLightShift.delay_time"
            ]

            reps = self.p["OneTimeMetastableMicrowaveRamseyLightShift.reps"]
            ...
            for i in np.arange(reps):
                should_break = self.update_progress(i / reps)
                if should_break:
                    break
                ...
                LS_heralded += len(counts_LS)
                C_heralded += len(counts_C)
                print(
                ...
                print("Total LS points heralded: ", LS_heralded)
                print("Total C points heralded: ", C_heralded)
