import labrad
from Qsim.scripts.pulse_sequences.metastable_rabi_qnd_point import (
    MetastableRabiQNDPoint as sequence,
)
...
class MetastableQubitQND(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        for i, duration in enumerate(self.times):
            ...
            [doppler_counts, herald_counts, qnd_counts, detection_counts] = (
            ...
            all_errors = np.unique(
                np.concatenate(
                    (failed_heralding[0], doppler_errors[0], qnd_to_ground_state[0])
                )
            )
