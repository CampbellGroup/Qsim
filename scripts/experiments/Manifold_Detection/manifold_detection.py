...
class ManifoldDetection(QsimExperiment):
        ...
        while still_there:
            for isdark in [True, False]:
                ...
                if self.current_fluorescence < 5.0:
                    ...
                    while (
                        time.time() - init_time < 300
                    ):  # or abs(self.current_fluorescence - self.fluorescence) < 1.0:
                        if self.current_fluorescence < 5.0:
                            print(
                                "Current fluorescence too low. Attempting to rescue ion"
                            )
                            if not self.rescue_ion(5.0):
                                ion_dead = True
                                break
                        ...
                        print(
                            "   current fluorescence is {}".format(
                                self.current_fluorescence
                            )
                        )
                        if isdark:
                            self.dv.add(
                                np.column_stack(
                                    (np.arange(len(counts)), np.array(counts))
                                ),
                    ...
                    if should_break or not still_there:
                        break
                    # if not self.correct_cavity_drift(): break

            if should_break or ion_dead:
                break

    def setup_datavault(self):
        self.dark_counts_context = self.dv.context()
        self.dv.cd(
            ["", "Manifold Detection Counts"], True, context=self.dark_counts_context
        )
        self.dark_counts_dataset = self.dv.new(
            "counts",
            [("run", "arb")],
        ...
        self.dv.cd(
            ["", "Manifold Detection Counts"], True, context=self.bright_counts_context
        )
        self.bright_counts_dataset = self.dv.new(
            "counts_bright",
            [("run", "arb")],
    ...
    def correct_cavity_drift(self):
        ...
        while np.abs(delta) > 0.2:
            if j > 10 or (
                self.cavity_voltage < self.cavity_rails[0],
            ...
            print(
                "Moving cavity from {} V to {} V".format(
