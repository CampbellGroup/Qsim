...
from Qsim.scripts.pulse_sequences.dark_state_preparation import (
    DarkStatePreparation as sequence,
)
...
class DarkStateDetection(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        while True:
            ...
            if mode == "Shelving":
                ...
                dark_dataset = np.column_stack(
                    (np.arange(len(counts)), np.array(counts), np.array(doppler_counts))
                )
                self.dv.add(dark_dataset, context=self.dark_state_counts)
                counts, total_doppler_errors = self.delete_doppler_count_errors(
                    doppler_counts, counts
                )

                print(
                    "Deleted"
                    + str(total_doppler_errors)
                    + " experiments due to Doppler errors"
                )
            ...
            if mode == "Shelving":
                if i == 1:
                    ...
                    if (
                        np.mean(doppler_counts)
                        < (self.hist_mean - 1.0 * self.hist_std_dev)
                    ) or (
                        np.mean(doppler_counts)
                        > (self.hist_mean + 1.0 * self.hist_std_dev)
                    ):
    ...
    def setup_prob_datavault(self):
        ...
        self.dark_state_counts_dataset = self.dv.new(
            "dark_state_counts",
            [("run", "arb")],
            [
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.dark_state_counts
            )
    ...
    def delete_doppler_count_errors(self, counts_doppler_dark, counts_dark):
        ...
        dark_delete = dark_delete[
            (dark_delete < len(counts_doppler_dark)) & (dark_delete >= 0.0)
        ]
    ...
    def run_interleaved_linescan(self):
        ...
        try:
            popt, pcov = fit(
                self.lorentzian_fit, detunings[2:], counts[2:], p0=fit_guess
            )
        except RuntimeError:
