import labrad
from Qsim.scripts.pulse_sequences.microwave_point.metastable_microwave_point_173 import (
    SweptMetastableMicrowavePoint173 as sequence,
)
...
class MetastableMicrowaveLineScanSwept173(QsimExperiment):
    """
    ...
    name = "Metastable Microwave Line Scan 173 Swept"

    exp_parameters = []
    exp_parameters.append(("DopplerCooling", "detuning"))
    exp_parameters.append(("Transitions", "main_cooling_369"))
    exp_parameters.append(("SweptMetastableMicrowaveLinescan173", "scan"))
    ...
    def run(self, cxn, context):
        ...
        self.detunings = self.get_scan_list(
            self.p.SweptMetastableMicrowaveLinescan173.scan, "kHz"
        )
        self.p["Metastable_Microwave_Interrogation.ramp_width"] = U(
            self.detunings[1] - self.detunings[0], "MHz"
        )
        self.p["Metastable_Microwave_Interrogation.duration"] = (
            self.p.Pi_times.metastable_qubit_173
        )
        ...
        for i, detuning in enumerate(self.detunings):
            ...
            self.dv.add(detuning + self.p.Transitions.MetastableQubit173["kHz"], pop)
...
if __name__ == "__main__":
    ...
    experiment = MetastableMicrowaveLineScanSwept173(cxn=cxn)
