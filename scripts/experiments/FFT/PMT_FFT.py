...
class PMT_FFT(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        for i in range(self.average):
            ...
            if should_break:
                break
            new_pwr = self.processor.getPowerSpectrum(
                self.freqs, timetags, self.record_time, U(10.0, "ns")
            )
    ...
    def set_scannable_parameters(self):
        ...
        self.freq = (
                detuning / 2.0 + self.p["Transitions.main_cooling_369"] / 2.0 + U(200.0, "MHz")
        )  # divide by 2 for the double pass
        ...
        self.freqs = self.processor.computeFreqDomain(
            self.record_time["s"],
            self.freq_span["Hz"],
            self.freq_offset["Hz"],
            self.center_freq["Hz"],
