...
class processFFT:
    """
    Mathematical operations useful for FFT processing.
    These are independent from LabRAD communication.
    """

    def computeFreqDomain(self, recordTime, freqSpan, freqOffset, centerFreq):
        """
        computers the domain of frequencies for the given recording time,
    ...
    def peakArea(self, spectrum, ptsAround):
        """
        Finds the maximum of the power spectrum,
        computers the area of the peak using ptsAround, and subtracts the background
        """
        ...
        background = (np.sum(spectrum) - peakArea) / (
            spectrum.size - (2 * ptsAround + 1)
        )
    ...
    def getPowerSpectrum(self, freqs, timetags, recordTime, timeResolution):
        ...
        mat = np.exp(-1.0j * 2.0 * np.pi * np.outer(freqs, timetags))
        fft = mat.sum(axis=1)
        pwr = np.abs(fft) ** 2.0
        if timetags.size > 0:
            # normalizes such that total of the complete power spectrum is 1:
            # timetags.size is the initial area of the signal while N is
            # the length of the array, see Parseval's theorem
