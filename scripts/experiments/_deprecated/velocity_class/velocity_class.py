...
class velocity_class(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        # Tickle on
        self.rg.applywaveform(
            "sine",
            self.tickle_freq,
            self.tickle_amplitude,
            WithUnit(0.0, "V"),
            self.rg_chan,
        )
        time.sleep(0.1)
    ...
    def process_data(self, column_sum):
        ...
        dataset = self.dv.new(
            "stacked_images" + " - " + str(self.path[1]),
            [("frequency", "MHz")],
            [("", "Intensity", "A.U.")],
            context=self.save_ctx,
        )
    ...
    def analyze(self, image):
        ...
        x_array = (
            x_array - x_array[-1] / 2.0
        )  # * 2 * np.pi * self.tickle_freq['MHz'] / (369.5e-9)
    ...
    def take_image(self, name):
        ...
        rotated_image = rotate(
            new_image, self.image_rotation_angle["deg"], reshape=False
        )
        rotated_row_image = rotated_image.reshape(-1)
        self.grapher.plot_image(
            rotated_row_image, self.data_size, "Images", self.path[1] + " " + name
        )
    ...
    def save_camera_data(self, image_data):
        ...
        self.dv.cd(self.path[1], True, context=self.save_ctx)
        self.dv.save_image(
            image_data, self.data_size, 1, self.path[1], context=self.save_ctx
        )
    ...
    def set_exp_settings(self):
        ...
        if self.background_subtraction == True:
            ...
            self.bg_image = self.bg_image.reshape(
                self.image_x_length, self.image_y_length
            )

        self.keithley.gpib_write("APPLy CH1," + str(self.ML_piezo_voltage["V"]) + "V")
        self.keithley.output(self.kt_chan, True)

        self.pulser.frequency(
            "369DP", self.WLcenter - WithUnit(10.0, "MHz") / 2.0
