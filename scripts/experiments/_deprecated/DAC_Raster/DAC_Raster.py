...
class dacRaster(Experiment):
    ...
    def initialize(self, cxn, context, ident):
        ...
        self.Xstepsize = (float(self.Xmaxval) - self.Xminval) / (
            self.Xnumberofsteps - 1
        )
        self.Xvalues = np.linspace(self.Xminval, self.Xmaxval, self.Xnumberofsteps)
        ...
        self.Ystepsize = int(
            (float(self.Ymaxval) - self.Yminval) / (self.Ynumberofsteps - 1)
        )
        self.Yvalues = np.linspace(self.Yminval, self.Ymaxval, self.Ynumberofsteps)
        ...
        self.Zstepsize = int(
            (float(self.DCmaxval) - self.DCminval) / (self.DCnumberofsteps - 1)
        )
        self.Zvalues = np.linspace(self.DCminval, self.DCmaxval, self.Znumberofsteps)

        self.totalstep = (
            (self.Xnumberofsteps) * (self.Ynumberofsteps) * (self.Znumberofsteps)
        )
