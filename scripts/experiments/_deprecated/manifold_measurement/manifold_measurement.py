import labrad
from Qsim.scripts.pulse_sequences.manifold_measurement import (
    manifold_measurement as sequence,
)
...
class manifold_measurement(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        while True:
            i += 1
            [counts_doppler_bright, counts_bright, counts_doppler_dark, counts_dark] = (
                self.run_sequence(max_runs=250, num=4)
            )

            bright_errors = np.where(
                counts_doppler_bright
                <= self.p.ShelvingStateDetection.doppler_counts_threshold
            )
    ...
    def setup_prob_datavault(self):
        ...
        self.dataset_prob = self.dv.new(
            ...
            [("run", "prob")],
            [("Prob", "bright_prep", "num"), ("Prob", "dark_prep", "num")],
        )
