import labrad
from Qsim.scripts.pulse_sequences.ML_bright_state_preparation import (
    ML_bright_state_preparation as bright_prep_seq,
)
from Qsim.scripts.pulse_sequences.ML_dark_state_preparation import (
    ML_dark_state_preparation as dark_prep_seq,
)
...
class ML_fidelity_tweak_up(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        while True:
            ...
            self.method = "bright"
            [counts_bright_cropped, counts_bright_all, timetags_bright] = (
                self.run_ML_sequence()
            )
            self.program_pulser(dark_prep_seq)
            self.method = "dark"
            [counts_dark_cropped, counts_dark_all, timetags_dark] = (
                self.run_ML_sequence()
            )
            ...
            self.plot_prob(
                i, counts_bright_cropped, counts_dark_cropped, context=self.cropped_ctx
            )
    ...
    def setup_prob_datavault(self):
        ...
        self.dataset_raw = self.dv.new(
            ...
            context=self.rawData_ctx,
        )

        self.dv.cd(
            ["", "ML_fidelity_tweak_up_timetags_bright"],
            True,
            context=self.timetags_bright_ctx,
        )

        self.dataset_timetags_bright = self.dv.new(
            "ML_fidelity_tweak_up",
            [("time_bin", "num")],
            [("Photons", "counts", "num")],
            context=self.timetags_bright_ctx,
        ...
        self.dv.cd(
            ["", "ML_fidelity_tweak_up_timetags_dark"],
            True,
            context=self.timetags_dark_ctx,
        )

        self.dataset_timetags_dark = self.dv.new(
            "ML_fidelity_tweak_up",
            [("time_bin", "num")],
            [("Photons", "counts", "num")],
            context=self.timetags_dark_ctx,
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.cropped_ctx
            )
    ...
    def plot_prob(self, num, counts_bright, counts_dark, context):
        ...
        self.dv.add(
            num, prob_dark, prob_bright, prob_bright - prob_dark, context=context
        )
