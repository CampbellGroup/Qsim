...
class metastable_transition(QsimExperiment):
    ...
    def run(self, cxn, context):
        self.forward_x_values = self.get_scan_list(
            self.p.metastable.M2_scan, units=None
        )
        self.reverse_x_values = list(
            reversed(self.forward_x_values[0:-1])
        )  # does not use turning point value
