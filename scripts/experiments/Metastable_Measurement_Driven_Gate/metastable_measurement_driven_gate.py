import labrad
from Qsim.scripts.pulse_sequences.metastable_measurement_driven_gate import (
    MetastableMeasurementDrivenGate as sequence,
)
...
class MetastableMeasurementDrivenGate(QsimExperiment):
    ...
    def run(self, cxn, context):
        ...
        [doppler_counts, herald_counts, failed_gate_counts, detection_counts] = (
            self.run_sequence(max_runs=250, num=4)
        )

        num = (
            np.ones(len(doppler_counts))
            * self.p.MetastableMeasurementDrivenGate.total_num_sub_pulses
        )
        self.dv.add(
            np.column_stack(
                (
        ...
        failed_gate = np.where(
            failed_gate_fixed >= self.p.ShelvingStateDetection.state_readout_threshold
        )
        detection_fixed = np.delete(detection_fixed, failed_gate)
        success_gate = np.where(
            detection_fixed >= self.p.ShelvingStateDetection.state_readout_threshold
        )

        print("Total number of heralded experiments = " + str(total_num_experiments))
        print(
            "Gate success rate = " + str(len(success_gate[0]) / total_num_experiments)
        )
    ...
    def setup_datavault_counts(self, x_axis, y_axis):
        ...
        self.dataset = self.dv.new(
            ...
            [(x_axis, "num")],
            [
                (y_axis, "Doppler_Counts", "num"),
                (y_axis, "Herald_Counts", "num"),
                (y_axis, "Shelving_Counts", "num"),
                (y_axis, "Metastable_Counts", "num"),
            ],
            context=self.counts_context,
        )
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.counts_context
            )
