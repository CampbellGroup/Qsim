import labrad
from Qsim.scripts.pulse_sequences.shelving_bright_spam import (
    ShelvingBrightSpam as bright_sequence,
)
from Qsim.scripts.pulse_sequences.shelving_dark_spam import (
    ShelvingDarkSpam as dark_sequence,
)
...
class HighFidelityMeasurement(QsimExperiment):
    ...
    exp_parameters = [("HighFidelityMeasurement", "sequence_iterations")]
    ...
    def run(self, cxn, context):
        ...
        self.init_dac_port_822_voltage = self.mp_server.get_output_voltage(
            self.dac_port_822
        )  # gets the init dac port voltage for the M2 lock
        ...
        while True:
            ...
            detection_delay_bright, detection_delay_dark = (
                self.get_detection_timetags_offset()
            )

            self.program_pulser(bright_sequence)
            [counts_doppler_bright, counts_bright], ttBright = (
                self.run_sequence_with_timetags(max_runs=250, num=2)
            )
            ...
            if np.abs(dac_voltage - self.init_dac_port_822_voltage) > 200.0:
                print(
                    "M2 Etalon hopped on experiment " + str(i) + ", killing experiment."
                )
                break
            ...
            ttBright = ttBright[0]
            ttB_dop, ttB_det = self.parse_timetags(
                ttBright, counts_doppler_bright, counts_bright
            )
            ttB_det = np.array(ttB_det) - detection_delay_bright
            self.save_bright_data(
                [counts_bright, counts_doppler_bright], [ttB_det, ttB_dop]
            )

            self.program_pulser(dark_sequence)
            [counts_doppler_dark, counts_dark], ttDark = (
                self.run_sequence_with_timetags(max_runs=250, num=2)
            )
            ...
            ttDark = ttDark[0]
            ttD_dop, ttD_det = self.parse_timetags(
                ttDark, counts_doppler_dark, counts_dark
            )
            ttD_det = np.array(ttD_det) - detection_delay_dark
            self.save_dark_data([counts_dark, counts_doppler_dark], [ttD_det, ttD_dop])

            countsBright, countsDark, n_errors = self.delete_doppler_count_errors(
                counts_doppler_bright, counts_doppler_dark, counts_bright, counts_dark
            )
    ...
    def cavity_tweak_up(self, i, countsBright):
        if i == 1:
            ...
            print(
                "mean bright detection counts on first experiment experiment is  = "
            ...
            if np.abs(diff) > 10.0:
                ...
                if np.sign(diff) * 0.005 < 0.2:
                    ...
                    print(
                        "Updated cavity voltage to " + str(self.cavity_voltage) + " V"
                    )
            else:
                pass
    ...
    def save_bright_data(self, counts, timetags):
        ...
        bright_data = np.column_stack(
            (
                np.arange(len(counts_bright)),
                np.array(counts_bright),
                np.array(counts_doppler_bright),
            )
        )
        self.dv.add(bright_data, context=self.hf_bright_context)
        self.dv.add(
            np.column_stack(
                (np.zeros(len(timetags_det_bright)), np.array(timetags_det_bright))
            ),
            context=self.tt_bright_det_context,
        )
        self.dv.add(
            np.column_stack(
                (np.zeros(len(timetags_dop_bright)), np.array(timetags_dop_bright))
            ),
    ...
    def save_dark_data(self, counts, timetags):
        ...
        dark_data = np.column_stack(
            (
                np.arange(len(counts_dark)),
                np.array(counts_dark),
                np.array(counts_doppler_dark),
            )
        )
        self.dv.add(dark_data, context=self.hf_dark_context)
        self.dv.add(
            np.column_stack(
                (np.zeros(len(timetags_det_dark)), np.array(timetags_det_dark))
            ),
            context=self.tt_dark_det_context,
        )
        self.dv.add(
            np.column_stack(
                (np.zeros(len(timetags_dop_dark)), np.array(timetags_dop_dark))
            ),
            context=self.tt_dark_dop_context,
        )

    def delete_doppler_count_errors(
        self, counts_doppler_bright, counts_doppler_dark, counts_bright, counts_dark
    ):
        """
        ...
        bright_errors = np.where(
            counts_doppler_bright
            <= self.p.Shelving_Doppler_Cooling.doppler_counts_threshold
        )
        bright_errors = np.unique(
            np.concatenate((bright_errors[0], bright_errors[0] - 1))
        )
    ...
    def setup_high_fidelity_datavault(self):
        ...
        self.dv.cd(
            ["shelving_fidelity", "shelving_probability_vs_runs"],
            True,
            context=self.prob_context,
        )

        self.dataset_prob = self.dv.new(
            "shelving_measurement",
            [("run", "prob")],
            [
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.prob_context
            )
        ...
        self.dv.cd(
            ["shelving_fidelity", "shelving_det_timetags_bright"],
            True,
            context=self.tt_bright_det_context,
        )
        self.tt_bright_det_dataset = self.dv.new(
            "timetags",
            [("arb", "arb")],
            [("time", "timetags", "num")],
            context=self.tt_bright_det_context,
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.tt_bright_det_context
            )

        self.tt_dark_det_context = self.dv.context()
        self.dv.cd(
            ["shelving_fidelity", "shelving_det_timetags_dark"],
            True,
            context=self.tt_dark_det_context,
        )
        self.tt_dark_det_dataset = self.dv.new(
            "timetags",
            [("arb", "arb")],
            [("time", "timetags", "num")],
            context=self.tt_dark_det_context,
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.tt_dark_det_context
            )

        self.tt_bright_dop_context = self.dv.context()
        self.dv.cd(
            ["shelving_fidelity", "shelving_dop_timetags_bright"],
            True,
            context=self.tt_bright_dop_context,
        )
        self.tt_bright_dop_dataset = self.dv.new(
            "timetags",
            [("arb", "arb")],
            [("time", "timetags", "num")],
            context=self.tt_bright_dop_context,
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.tt_bright_dop_context
            )

        self.tt_dark_dop_context = self.dv.context()
        self.dv.cd(
            ["shelving_fidelity", "shelving_dop_timetags_dark"],
            True,
            context=self.tt_dark_dop_context,
        )
        self.tt_dark_dop_dataset = self.dv.new(
            "timetags",
            [("arb", "arb")],
            [("time", "timetags", "num")],
            context=self.tt_dark_dop_context,
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.tt_dark_dop_context
            )
        ...
        self.dv.cd(
            ["shelving_fidelity", "shelving_counts_bright"],
            True,
            context=self.hf_bright_context,
        )
        self.hf_bright_dataset = self.dv.new(
            "counts",
            [("run", "arb")],
            [
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.hf_bright_context
            )

        self.hf_dark_context = self.dv.context()
        self.dv.cd(
            ["shelving_fidelity", "shelving_counts_dark"],
            True,
            context=self.hf_dark_context,
        )
        self.hf_dark_dataset = self.dv.new(
            "counts",
            [("run", "arb")],
            [
                ("counts", "detection_counts", "num"),
        ...
        for parameter in self.p:
            self.dv.add_parameter(
                parameter, self.p[parameter], context=self.hf_dark_context
            )

        self.pi_time_context = self.dv.context()
        self.dv.cd(
            ["shelving_fidelity", "shelving_rabi_tracking"],
            True,
            context=self.pi_time_context,
        )
        self.rabi_tracking_dataset = self.dv.new(
            "rabi_tracking",
            [("time", "s")],
            [("pi_time", "", "us")],
            context=self.pi_time_context,
    ...
    def get_detection_timetags_offset(self):
        ...
        if op_method == "Standard":
            op_time = (
                self.p.OpticalPumping.duration + self.p.OpticalPumping.extra_repump_time
            )
        elif op_method == "Both":
            op_time = (
                self.p.OpticalPumping.duration
                + self.p.OpticalPumping.quadrupole_op_duration
                + self.p.OpticalPumping.extra_repump_time
            )
            ...
            print(
                "You have selected QuadrupoleOnly optical pumping, rethink your choices"
            )

        uW_method = self.p.MicrowaveInterrogation.PulseSequence
        if uW_method == "standard":
            uW_duration = (
                self.p.MicrowaveInterrogation.duration
                + self.p.MicrowaveInterrogation.ttl_switch_delay
            )
        elif uW_method == "knill":
            uW_duration = (
                5.0 * self.p.MicrowaveInterrogation.duration
                + 5.0 * self.p.MicrowaveInterrogation.ttl_switch_delay
            )
        elif uW_method == "SpinEcho":
            uW_duration = (
                2.0 * self.p.MicrowaveInterrogation.duration
                + 3.0 * self.p.MicrowaveInterrogation.ttl_switch_delay
            )
        elif uW_method == "SuSequence":
            uW_duration = 5.0 * self.p.MicrowaveInterrogation.duration + 2 * U(
                15.0, "us"
            )
        elif uW_method == "DoubleStandard":
            uW_duration = (
                self.p.Pi_times.qubit_0 + self.p.Pi_times.qubit_plus + U(200.0, "us")
            )
        else:
            uW_duration = U(0.0, "us")
            print(
                "You have not selected a microwave sequence that is in the timetags offset"
            )
